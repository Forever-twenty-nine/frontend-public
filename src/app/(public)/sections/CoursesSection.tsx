...
const CoursesSection: React.FC<{ className?: string }> = ({ className }) => {
            ...
            <section className={`bg-white py-12 md:py-16 ${className || ""}`}>
                <div className="container mx-auto px-4">
                    <div className="flex flex-col items-center justify-center rounded-lg bg-red-50 py-16 text-center">
                        <svg className="mb-4 h-16 w-16 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <p className="mb-4 text-lg text-red-600">{error}</p>
                        <button onClick={() => window.location.reload()} className="relative inline-block px-6 py-3 text-base font-semibold mt-4 cursor-pointer text-brand-primary rounded-full transition duration-500 bg-transparent border border-solid border-brand-primary hover:bg-brand-primary/10 hover:ring-[3px] active:bg-brand-tertiary active:text-white active:ring-brand-secondary">
                            Reintentar
                        </button>
                    </div>
                </div>
    ...
    return (
        <section className={`bg-linear-to-b from-brand-tertiary/5 to-brand-tertiary/20 py-10 pb-16 ${className || ""}`} aria-label="Cursos más solicitados">
            <div className="max-w-7xl mx-auto px-4">
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                        <svg className="mb-4 h-20 w-20 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {!loading && courses.length > 0 && (
                    <div className="mt-8 flex justify-center md:mt-12">
                        <Link href="/cursos" className="group inline-flex items-center gap-2 rounded-full bg-brand-tertiary px-6 py-3 text-base font-bold text-white shadow-2xl transition-all duration-500 hover:bg-brand-primary-dark hover:text-white hover:ring-4 hover:ring-brand-primary active:bg-brand-tertiary active:text-white active:ring-brand-secondary focus:outline-none focus:ring-4 focus:ring-brand-primary">
